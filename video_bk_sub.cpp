...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        // Filtrado de mediana para reducir ruido
        //cv::medianBlur(grayFrame, grayFrame, 5);

        // Filtrado gaussiano para suavizar la imagen y reducir ruido
        cv::GaussianBlur(grayFrame, 
                         grayFrame, 
                         cv::Size(5, 5), 
                         0.5, /*Gaussian kernel standard deviation in X direction.*/
                         0.5  /*Gaussian kernel standard deviation in Y direction.*/
                        );
        ...
        double threshold = mean[0] + 7 * stddev[0];
